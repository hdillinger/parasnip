function explore( varargin )%data, displ, loop )
%EXPLORE Let's you interactively explore multi-dimensional data sets 
%   
%   explore(data): Displays the first two dimensions of data as imagesc and loops
%       through all non-singleton dimensions
%
%   explore(data,displ): Displays the dimensions of data specified in displ and
%       loops through all non-singleton dimensions
%
%   explore(data,displ,loop): Displays the dimensions of data specified in
%       displs and loops through dimensions specified in loop
%
%   Possible actions:
%   - UP-Arrow:   Go to next plot or add subplot
%   - DOWN-Arrow: Go to previous subplot
%   - DELETE:     Delete active plot
%   - LEFT-Arrow: Decrease loop index of active plot (SHIFT increases all)
%   - RIGHT-Arrow:Increase loop index of active plot (SHIFT increases all)
%   - `:          Toggle display mode (abs, real, imag, angle) (SHIFT
%                 copies from active)
%   - 1:          Set active plot mode to abs (SHIFT sets all)
%   - 2:          Set active plot mode to real (SHIFT sets all)
%   - 3:          Set active plot mode to imaginary (SHIFT sets all)
%   - 4:          Set active plot mode to phase (SHIFT sets all)
%   - a:          Toggle axis mode (default, square, equal, tight) (SHIFT
%                 copies from active)
%   - b:          Toggle colorbar mode (on, off) (SHIFT copies from active)
%   - m:          Toggle colormap (default, gray, ...) (SHIFT copies from 
%                 active)
%   - l:          Toggle log plot (on, off) (SHIFT copies from active)
%   - c:          Copies active plot indices and processing code to
%                 clipboard
%   - f:          Toggles title color for active plot (blue, black)
%   - ESCAPE:     Closes explore window
%   - r:          Draw a ROI on the current figure and save the matrix to
%                 roi 
%   - p:          Publication mode, remove all axis labels
%
% AUTHOR: Christian Guenthner, guenthner@biomed.ee.ethz.ch
% STARTED: 2016-06-05
% VERSION: 2016-07-03-001
set(0,'defaultfigurecolor',[1 1 1]);
if nargin == 0
     error('explore must be called with at least one argument');
end

data = varargin{1};
nonumarg=1;
%automatically cat data
for i=2:length(varargin)
    if isnumeric(varargin{i}) || islogical(varargin{i})
        data = cat(2,data,varargin{i});
        nonumarg=nonumarg+1;
    else
        break;
    end
...
displ = [1 2];
loop = 3:length(size(data));
%
axismodes = {'normal','square','equal','tight'};
...
map4 = interp1([0;.5;1],[133, 52, 59; 255, 255, 255; 52, 56, 133]/255,linspace(0,1,256));
...
custommaps = {'eth','eth1','eth2','diff','ampl','phase','grayhires','bw','inferno','fake_parula','magma','plasma','viridis'};
colormaps = {'jet','parula','default','gray','hot','copper','pink','hsv',map1,map2,map3,map4,map5,map6,map7,map8,map9,inferno(256),fake_parula(256),magma(256),plasma(256),viridis(256)};
preprocs = {@(x)abs(x),@(x)real(x),@(x)imag(x),@(x)angle(x)};
preprocstrs = {'magnitude','real-part','imaginary-p art','phase'};
preproccodes = {'abs','real','imag','angle'};
...
def_preproc = 1;
def_axismode = 1;
def_cm_index = length(colormaps)-5-5;
def_cbar = 0;
def_plotlog = 0;
def_title = '';
def_repeat = 0;
...
if nargin > nonumarg
    if mod(nargin-nonumarg,2) ~= 0
        error('data needs to be followed by an even number of arguments');
    end
    
    for n=1:(nargin-nonumarg)/2
        aid=2*n+nonumarg-1;
        %test type of argument
        switch( varargin{aid} )
            case 'preapply'
                % apply a processing step to the full data matrix
                data = varargin{aid+1}(data);
            case 'repeat'
                def_repeat = varargin{aid+1};
            case 'displ'
                displ = varargin{aid+1};
                loop = setdiff(1:length(size(data)),displ);
            case 'loop'
                loop = varargin{aid+1};
            case 'caxis'
                fixed_colorbar = 1;
                fixed_colorbar_values = varargin{aid+1};
            case 'figure'
                def_figure = varargin{aid+1};
            case 'colormap'
                cm = varargin{aid + 1};
                if ~ischar(cm)
                    def_cm_index = cm;
            ...
            case 'axismode'
                ...
                if ~ischar(cm)
                    ...
                    cm2 = find(strcmpi(cm,axismodes));
                    if isempty(cm2)
                        error('unknown axismode "%s". Use one of %s',cm,strjoin(axismodes,', '));
                    end
                    def_axismode = cm2(1);
                end
            case 'colorbar'
                def_cbar = varargin{aid+1};
            case 'preproc'
                cm = varargin{aid + 1};
                if ~ischar(cm)
                    def_preproc = cm;
                else
                    cm2 = find(strcmpi(cm,preprocstrs));
                    if isempty(cm2)
                        cm2 = find(strcmpi(cm,preproccodes));
                        if isempty(cm2)
                            error('unknown preprocessing "%s". Use one of %s',cm,strjoin(preproccodes,', '));
                        end
                    end
                    def_preproc = cm2(1);
                end
            case 'title'
                def_title = varargin{aid+1};
            case 'pubmode'
                def_pubmode = varargin{aid+1};
            otherwise
...
rest = setdiff(1:length(size(data)),[displ loop]);
data_rearr = permute(data,[displ loop rest]);
roi_rearr = zeros(size(data_rearr));
sz = size(data);
...
frame = 1;
pubmode = def_pubmode;
explore_setdefaults(curplot);
%

maxindex = size(data_rearr);
if(length(maxindex)~=length(sz)); maxindex(end+1:length(sz)) = 1; end
maxindex = prod([1 maxindex(3:2+length(loop))]);

if isempty(def_figure)
    fh = figure;
else
    fh = figure(def_figure);
end
set(fh,'color',[1 1 1])
set(fh,'KeyPressFcn',@(fh,evt)update_explore(fh,evt));

update_explore(fh);

function explore_setdefaults(cp)
    if cp > 1
        curind{cp} = curind{cp-1};
        preproc{cp} = preproc{cp-1};
        axismode{cp} = axismode{cp-1};
        cm_index{cp} = cm_index{cp-1};
        cbar{cp} = cbar{cp-1};
        plotlog{cp} = plotlog{cp-1};
    else
        curind{cp} = def_index;
        preproc{cp} = def_preproc;
        axismode{cp} = def_axismode;
        cm_index{cp} = def_cm_index;
        cbar{cp} = def_cbar;
        plotlog{cp} = def_plotlog;
    end
end

function explore_deleteplot()
    cp = curplot;
    curind(cp) = [];
    preproc(cp) = [];
    axismode(cp) = [];
    ...
    cbar(cp) = [];
    plotlog(cp) = [];
    curplot = min(cp,length(curind));
end

function update_explore(fh,event)
    noplot = 0;
    if nargin == 2
        switch event.Key
        case 'uparrow'  % go to next plot or add new
            curplot = curplot + 1;
            if curplot > length(curind)
                explore_setdefaults(curplot);
            end
        case 'downarrow' % go to previous plot
            curplot = max(1,curplot-1);
        case 'delete' % deletes active plot
            if length(curind)>1
                explore_deleteplot();
            end            
        case 'rightarrow' %increase explore count (SHIFT increments all)
            if def_repeat
                if ismember('shift',event.Modifier)
                    for m=1:length(curind); curind{m} = wrap(curind{m} + 1,1,maxindex);end
                else
                    curind{curplot} = wrap(curind{curplot} + 1,1,maxindex);
                end
            else
                if ismember('shift',event.Modifier)
                    for m=1:length(curind); curind{m} = min(curind{m} + 1,maxindex);end
                else
                    curind{curplot} = min(curind{curplot} + 1,maxindex);
                end
            end
        case 'leftarrow' %decrease explore count (SHIFT decrements all)
            if def_repeat
                if ismember('shift',event.Modifier)
                    for m=1:length(curind); curind{m} = wrap(curind{m} - 1,1,maxindex);end
                else
                ...
                if ismember('shift',event.Modifier)
                    for m=1:length(curind); curind{m} = max(curind{m} - 1,1);end
                else
                    curind{curplot} = max(1,curind{curplot} - 1);
                end
                
            end
        case 'backquote' %toggle plot mode (SHIFT copies from active plot, CNTRL toggles all) 
            preproc = change_option(event,preproc,curplot,preprocs);
        case '1' %abs plot (SHIFT sets all)
            if ismember('shift',event.Modifier); for m=1:length(curind); preproc{m} = 1; end; else preproc{curplot} = 1; end
        case '2' %real plot (SHIFT sets all)
            if ismember('shift',event.Modifier); for m=1:length(curind); preproc{m} = 2; end; else preproc{curplot} = 2; end
        case '3' %imag plot (SHIFT sets all)
            if ismember('shift',event.Modifier); for m=1:length(curind); preproc{m} = 3; end; else preproc{curplot} = 3; end
        case '4' %phase plot (SHIFT sets all)
            if ismember('shift',event.Modifier); for m=1:length(curind); preproc{m} = 4; end; else preproc{curplot} = 4; end
        case {'a','A'} %axismode (SHIFT copies from active plot, CTRL toggles all)
            axismode = change_option(event,axismode,curplot,axismodes);
        case {'m','M'} %colormap (SHIFT copies from active plot, CTRL toggles all)            
            cm_index = change_option(event,cm_index,curplot,colormaps);
        case {'b','B'} %add colorbar (SHIFT copies from active plot, CTRL toggles all)
            cbar = change_option(event,cbar,curplot,2,'bool');
        case 'f' %change color of current plot when active (GLOBAL)
            frame = mod(frame+1,2);
        case {'l','L'} %log plot (SHIFT copies from active plot)
            plotlog = change_option(event,plotlog,curplot,2,'bool');
        case 'c' %copy current plot code to clipboard 
            if ismember('control',event.Modifier)
                if plotlog{curplot}
                    clipboard('copy',['log(abs(' preproccodes{preproc{curplot}} '(squeeze(data(' mkindexstr(curplot) ')))))'])
                else
                    clipboard('copy',[preproccodes{preproc{curplot}} '(squeeze(data(' mkindexstr(curplot) ')))'])
                end
...
%              disp(event.Modifier);
        case 'r'
            tmproi=roipoly;
            roi_rearr(:,:,curind{n}) = tmproi;
            tmproi = ipermute(roi_rearr,[displ loop rest]);
            assignin('base','roi',tmproi);
        case 'p' %Pubmode remove axis
            pubmode = ~pubmode;%change_option(event,plotlog,curplot,2,'bool');
        end    
    end
    
    if ~noplot
    figure(fh);
    [col, row] = subplotdim(length(curind),4/3);
    for n=1:length(curind)
        h=subplot(row,col,n);
        if plotlog{n}
            imagesc(squeeze(log10(abs(preprocs{preproc{n}}(data_rearr(:,:,curind{n}))))));
        else
            if ~isnumeric(preproc{n})
                imagesc(squeeze(preproc{n}(data_rearr(:,:,curind{n}))));
            else
                imagesc(squeeze(preprocs{preproc{n}}(data_rearr(:,:,curind{n}))));
            end
        end
        axis(axismodes{axismode{n}});
        colormap(h,colormaps{cm_index{n}});
        if fixed_colorbar
            caxis(fixed_colorbar_values);
        else
            caxis auto; 
        end
        if cbar{n}
        ...
        % create title
        if ~isnumeric(preproc{n})
            str = ['custom [' mkindexstr(n) ']'];
        else
            str = [preprocstrs{preproc{n}} '[' mkindexstr(n) ']'];
        end
        if ~isempty(def_title)
            str = [def_title '-' str];
        end
        if and(frame,n==curplot)
            title(['\color[rgb]{0 .5 .5}' str]);
        else
            title(str);
        end
...
function str=mkindexstr(n)
    c=cell(length(sz));
    c{displ(1)}=':';
    c{displ(2)}=':';
    IS = ind2subarr(sz(loop),curind{n});
    for m=1:length(loop)
        c{loop(m)} = num2str(IS(m));
    end
    for m=1:length(rest)
        c{rest(m)} = '1';
    end
...
% function [ind] = subarr2ind(sz,arr)
%     sz = [1 sz];
%     ind = 1;
%     for n=1:length(arr)
%         ind = ind + prod(sz(1:n))*(arr(n)-1);
%     end
% end

function c = change_option(event, c,curplot,list,mode)
    if nargin == 4
        mode = 'def';
    end
    if ~ismember('shift',event.Modifier)
        if ~isnumeric(c{curplot}) && ~islogical(c{curplot})
            c{curplot}=0;
        else
            if strcmp(mode,'bool')
                c{curplot} = mod(c{curplot}+1,2);
            else
                c{curplot} = mod(c{curplot},length(list))+1;
            end
        end
    end
    if ~isempty(event.Modifier)
        for m=1:length(c); c{m} = c{curplot}; end
    end
end

function [arr] = ind2subarr(sz,ind)
    sz = [1 sz];
    arr = zeros(size(sz));
    for n=length(sz):-1:1
        arr(n) = floor((ind-1)/prod(sz(1:n)))+1;
        ind = ind - prod(sz(1:n))*(arr(n)-1);
    end
end

function [col,row] = subplotdim(n,aspect)
    col = round(sqrt(aspect)*sqrt(n));
    row = ceil(n/col);
end

function x = wrap(x,min,max)
    if x>max
        x = min;
    elseif x<min
        x = max;
